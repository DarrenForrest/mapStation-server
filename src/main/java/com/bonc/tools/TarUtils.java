...
public abstract class TarUtils {
	 ...
	 * @throws Exception
	 */
	public static void append(String srcPath, String destFile) throws Exception {
		File tarFile = new File(destFile);
		File tmpFile = new File(destFile + ".tmp");
		TarArchiveInputStream tais = new TarArchiveInputStream(new FileInputStream(tarFile));
		TarArchiveOutputStream taos = new TarArchiveOutputStream(new FileOutputStream(tmpFile));
		for(TarArchiveEntry i = tais.getNextTarEntry(); i != null; i = tais.getNextTarEntry()) {
			taos.putArchiveEntry(i);
			IOUtils.copy(tais, taos);
			taos.closeArchiveEntry();
		}
		tais.close();
		
		archive(new File(srcPath), taos, BASE_DIR);
